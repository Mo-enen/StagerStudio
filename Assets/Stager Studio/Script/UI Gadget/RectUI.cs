namespace StagerStudio.UI {
	...
	public class RectUI : Image {
		...
		private void PopulateSlicedMesh (VertexHelper toFill) {
			...
			VertexCache[0].color = VertexCache[1].color = VertexCache[2].color = VertexCache[3].color = color;
			if (sprite is null) {
				VertexCache[0].uv0 = new Vector2(0, 0);
				VertexCache[1].uv0 = new Vector2(0, 1);
				VertexCache[2].uv0 = new Vector2(1, 1);
				VertexCache[3].uv0 = new Vector2(1, 0);
			} else {
				VertexCache[0].uv0 = sprite.uv[1];
				VertexCache[1].uv0 = sprite.uv[3];
				VertexCache[2].uv0 = sprite.uv[2];
				VertexCache[3].uv0 = sprite.uv[0];
			}
			foreach (var r in m_Rects) {
		...
		private void GenerateSlicedSprite (VertexHelper toFill, Rect pRect, Rect rect) {
			...
			Vector4 adjustedBorders = GetAdjustedBorders(border / multipliedPixelsPerUnit, pRect);
