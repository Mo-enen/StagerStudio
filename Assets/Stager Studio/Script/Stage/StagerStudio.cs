namespace StagerStudio {
	...
	public partial class StagerStudio : MonoBehaviour {
		...
		public delegate (bool use, bool all, int index, float width) AbreastHandler ();
		...
		private void Awake () {
			...
			GetAbreastData = () => (game.UseAbreast, game.AllStageAbreast, game.AbreastIndex, game.AbreastWidth);
		...
		private void Update () {
			var (useA, allA, aIndex, aWidth) = GetAbreastData();
			...
			StageObject.Abreast = (aIndex, useA, allA);
		...
		private void RefreshAuthorLabel () {
			...
			var mapMap = ProjectInfoUI.GetBeatmapMap();
			var key = ProjectInfoUI.GetBeatmapKey();
			Beatmap map = null;
