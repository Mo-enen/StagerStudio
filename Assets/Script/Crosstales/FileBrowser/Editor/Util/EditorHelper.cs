...
namespace Crosstales.FB.EditorUtil
{
   ...
   public abstract class EditorHelper : Common.EditorUtil.BaseEditorHelper
   {
      ...
      public static void FBUnavailable()
      {
         ...
         EditorGUILayout.HelpBox($"Did you add the '{Util.Constants.FB_SCENE_OBJECT_NAME}'-prefab to the scene?", MessageType.Info);

         GUILayout.Space(8);

         if (GUILayout.Button(new GUIContent($"Add {Util.Constants.FB_SCENE_OBJECT_NAME}", Icon_Plus, $"Add the '{Util.Constants.FB_SCENE_OBJECT_NAME}'-prefab to the current scene.")))
         {
            InstantiatePrefab(Util.Constants.FB_SCENE_OBJECT_NAME);
         }
      }
      ...
      public static bool isFileBrowserInScene => GameObject.FindObjectOfType(typeof(FileBrowser)) != null; //GameObject.Find(Util.Constants.FB_SCENE_OBJECT_NAME) != null;
