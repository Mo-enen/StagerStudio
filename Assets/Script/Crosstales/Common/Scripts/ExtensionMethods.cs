...
*/
      ...
      /// <param name="encoding">Encoding of the string (default: UTF8, optional).</param>
      /// <returns>Byte-array with the string.</returns>
      public static string CTToByteArray(this byte[] data, System.Text.Encoding encoding = null)
      {
         if (data == null)
            return null;
      ...
      public static Bounds CTGetBounds(this RectTransform transform, float uiScaleFactor = 1f)
      {
         ...
         Bounds bounds = new Bounds(transform.anchoredPosition, new Vector3(transform.rect.width, transform.rect.height, 0.0f) * uiScaleFactor);

         if (transform.childCount > 0)
         {
            foreach (RectTransform child in transform)
            {
               //Debug.Log($"Child: {child.anchoredPosition}");
               Bounds childBounds = new Bounds(child.anchoredPosition, new Vector3(child.rect.width, child.rect.height, 0.0f) * uiScaleFactor);
      ...
      public static Vector4 CTGetLRTB(this RectTransform transform)
      {
         ...
         return new Vector4(transform.offsetMin.x, transform.offsetMax.x, transform.offsetMax.y, transform.offsetMin.y);
      }
